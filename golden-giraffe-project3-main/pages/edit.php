<?php
define("MAX_FILE_SIZE", 100000000);

require_once 'includes/sessions.php';
process_session_params($db, $session_messages);
if (is_user_logged_in()) {
    $upload_source = NULL;
    $upload_file_name = NULL;
    $upload_file_ext = NULL;

    $entry_id = $_GET['id'];

    $form_values = array(
        'title' => '',
        'description' => '',
        'tags' => ''
    );

    $sticky_values = array(
        'title' => '',
        'description' => '',
        'tags' => ''
    );

    $upload_feedback = array(
        "general_error" => false,
        "too_large" => false
    );

    $res = exec_sql_query(
        $db,
        "SELECT * FROM tags"
    );

    $all_tags = $res->fetchAll();

    if (isset($_POST['submit_info'])) {
        $form_valid = true;

        $tag_values = array();
        foreach ($all_tags as $tag) {
            $tag_values[$tag["id"]] = isset($_POST[$tag["tag_name"]]);
        };

        $form_values["title"] = ($_POST['title'] == "" ? NULL : trim($_POST['title']));
        $form_values["description"] = ($_POST['description'] == "" ? NULL : trim($_POST['description']));

        $sticky_values['title'] = htmlspecialchars($form_values['title']);
        $sticky_values['description'] = htmlspecialchars($form_values['description']);
        $sticky_values['tags'] = htmlspecialchars($form_values['tags']);

        $upload_source = trim($_POST["source"]);
        if ($upload_source == "") {
            $upload_source = NULL;
        };

        $upload = $_FILES["jpg-file"];

        if (!in_array("true", $tag_values)) {
            $form_valid = false;
        }

        if ($form_values['title'] == "") {
            $form_valid = false;
            $form_values['title'] = "";
        }

        if ($form_values['description'] == "") {
            $form_valid = false;
            $form_values['description'] = "";
        }

        if ($upload["error"] != UPLOAD_ERR_OK) {
            $form_valid = false;
            if (($upload["error"] == UPLOAD_ERR_INI_SIZE) || ($upload["error"] == UPLOAD_ERR_FORM_SIZE)) {
                $upload_feedback["too_large"] = true;
            } else {
                $upload_feedback["general_error"] = true;
            }
        }

        if ($form_valid) {
            $update = exec_sql_query(
                $db,
                "UPDATE posts SET title = :title, description = :description, img_source = :img_source WHERE id = :id",
                array(
                    ':title' => $form_values["title"],
                    ':description' => $form_values["description"],
                    ':img_source' => $upload_source,
                    ':id' => $entry_id
                )
            );

            $delete_tags = exec_sql_query(
                $db,
                "DELETE FROM post_tags WHERE post_id = :id",
                array(':id' => $entry_id)
            );

            foreach ($tag_values as $tag_id => $value) {
                if ($value) {
                    $insert_tags = exec_sql_query(
                        $db,
                        "INSERT INTO post_tags(post_id, tag_id) VALUES (:post_id, :tag_id)",
                        array(
                            ':post_id' => $entry_id,
                            ':tag_id' => $tag_id
                        )
                    );
                }
            }

            $upload_file_name = basename($upload["name"]);
            $upload_file_ext = strtolower(pathinfo($upload_file_name, PATHINFO_EXTENSION));

            if (!in_array($upload_file_ext, array("jpg"))) {
                $form_valid = false;
                $upload_feedback["general_error"] = true;
            } else {
                $upload_storage_path = 'public/uploads/posts/' . $entry_id . '.' . $upload_file_ext;
                if (!move_uploaded_file($upload["tmp_name"], $upload_storage_path)) {
                    error_log("Failed to permanently store the uploaded file on the file server. Please check that the server folder exists.");
                }
            }
        }
    }

    if ($entry_id) {
        $post_query = "SELECT * FROM posts WHERE id = :id";
        $post_params = array(':id' => $entry_id);
        $post_result = exec_sql_query($db, $post_query, $post_params);

        if ($post_result) {
            $post = $post_result->fetch();

            $tags_query = "SELECT tag_name FROM tags INNER JOIN post_tags ON tags.id = post_tags.tag_id WHERE post_tags.post_id = :entry_id";
            $tags_result = exec_sql_query($db, $tags_query, array(':entry_id' => $entry_id));
            $tags = $tags_result->fetchAll();
        } else {
            $post = null;
        }
    } else {
        $post = null;
    };

    $file_url = '/public/uploads/posts/' . $entry_id . '.jpg';
}

?>

<!DOCTYPE html>
<html lang="en">

<head>
    <link rel="StyleSheet" type="text/css" href="public/styles/site.css" />
    <!-- Source:https://fonts.google.com/specimen/Comfortaa?query=Comfortaa&classification=Display&stylecount=2 -->
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Comfortaa:wght@300..700&display=swap" rel="stylesheet">

    <!-- Source:https://fonts.google.com/specimen/Pinyon+Script-->
    <link href="https://fonts.googleapis.com/css2?family=Pinyon+Script&display=swap" rel="stylesheet">

    <link rel="stylesheet" href="https://fonts.googleapis.com/css2?family=Material+Symbols+Outlined:opsz,wght,FILL,GRAD@24,400,0,0" />
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">

    <title>DRESS 2 EMPRESS</title>
</head>

<body>
    <?php include 'includes/header.php'; ?>

    <?php if (is_user_logged_in()) { ?>
        <a href="<?php echo logout_url(); ?>" class="logout-button">Log Out</a>
        <?php if ($post) { ?>
            <form class="edit-form" action="/edit?id=<?php echo htmlspecialchars($entry_id); ?>" method="post" enctype="multipart/form-data" novalidat>
                <h2>Edit Entry: <?php echo htmlspecialchars($post['title']); ?></h2>

                <label for="title">Title:</label>
                <input type="text" id="title" name="title" value="<?php echo htmlspecialchars($post['title']); ?>"><br>

                <label for="description">Description:</label>
                <textarea id="description" name="description"><?php echo htmlspecialchars($post['description']); ?></textarea><br>


                <label for="request-tag">Tags</label>
                <div class="tag_cols">
                    <?php foreach ($all_tags as $tag) { ?>
                        <div>
                            <input name="<?php echo $tag['tag_name']; ?>" id="request-tag" type="checkbox" value='<?php echo $tag['tag_name']; ?>'> <?php echo $tag['tag_name']; ?> </input>
                        </div>
                    <?php } ?>

                </div>
                </div>


                <label for="media">Media:</label>
                <div class="media-placeholder">
                    <img src="<?php echo htmlspecialchars($file_url) ?>" alt="Media placeholder">
                </div>

                <div class="form_size">
                    <input type="hidden" name="MAX_FILE_SIZE" value="<?php echo MAX_FILE_SIZE; ?>">

                    <?php if ($upload_feedback["too_large"]) { ?>
                        <p class="feedback">Unfortunately, the file you chose failed to upload because it was too big. Please select a file that is no larger than 10 MB.</p>
                    <?php } ?>

                    <?php if ($upload_feedback["general_error"]) { ?>
                        <p class="feedback">Oh No! Something went wrong. Please select an JPG file to upload.</p>
                    <?php } ?>
                </div>

                <div class="form_upload">
                    <label for="request-upload"> Upload JPG File:</label>
                    <input id="request-upload" type="file" name="jpg-file" accept=".jpg,image/jpg+xml">
                </div>

                <div class="form_source">
                    <label for="upload-source" class="optional">Source URL:</label>
                    <input id="upload-source" type="url" name="source" placeholder="URL where found. (optional)">
                </div>

                <div class="form-footer">
                    <input type="submit" value="Submit Changes" class="submit-button" name="submit_info">
                </div>

            </form>
        <?php } else { ?>
            <p>Entry not found. Please check the ID or <a href="/profile">return to the admin page</a>.</p>
        <?php } ?>
        <a class="back-button" href="/profile">Back to Admin Page</a>
    <?php } else { ?>
        <p>Please login to view this page.</p>
        <?php echo login_form('/profile', $session_messages); ?>
    <?php } ?>
</body>

</html>
